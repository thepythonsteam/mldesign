...
#         self.end_page = end_page
    ...
    def parse_page(self, html: str):
        ...
        try:
            ...
            current_page = None
        t = soup.findAll('script', {'type': 'text/javascript'})[3].decode_contents().strip().split("window._cianConfig['legacy-commercial-serp-frontend'] = ")[0]
        t = t[0:-2]
        ads = json.loads(t[[m.start() for m in re.finditer('"offers"', t)][0] + 9: ].split("paginationUrls")[0][:-2])
        for res in ads:
            try:
                photos = []
